...
are used in the DPLL algorithm only.
...
additional arguments for accumulating the choices and assignment.
...
literal is chosen from the set of clauses and the set is partially
evaluated for this assignment by the predicate \p{evaluate}. Both
predicates use \p{eliminate} to delete clauses that contain the literal
and \p{delete\_complement} to delete occurrences of the complement of
the literals.

The module \p{dpll} contains its own version of the predicate
\p{put\_clauses}; it displays the set of clauses with each clause on a
separate row. If \p{set\_verbose} has been run, the algorithm will
print: (1) the set of clauses that result from unit propagation, (2) the
clauses that are deleted and (3) the literals deleted from each clause.

The test program \p{dpll-queens-t.pro} is for the four-queens problem
given in the book. If the literal entered is \p{p12}, the predicate
terminates with the empty set of clauses and the assignments to positive
literals give the answer. Similarly, a solution is obtained if the
literal entered is \p{p13}. If the literals entered are, for example,
\p{p11} followed by \p{p23}, the predicate terminates with a set
containing the empty clause, showing that the set is unsatisfiable. The
test program also contains the encoding of the four-queens problem
described in Exercise 1.

The test program \p{dpll-tseitin-t.pro} contains the Tseitin clauses for
the graphs $K_{2,2}$, $K_{3,3}$ and the example used in Section 4.5 of
the book. The sets of clauses are, of course, unsatisfiable. Be careful
with the clauses for $K_{3,3}$, as a search takes quite a while and
generates a very large amount of output. For each set of clauses, a
...
literals) and writes it to a file in \emph{simplified DIMACS cnf format}
that is used by SAT solvers.
