...
traces the computation of a dominator, and the options \p{learned},
\p{resolvent} and \p{uip} trace the computation by resolution.
...
A node (here, the node \p{x4=1@3}) is called a \emph{dominator}
of a decision node if it appears on all the paths from the decision node
to \p{kappa}. \ls{} computes the paths and finds a dominator:
\begin{verbatim}
...
the current node clashes with the current clause on the literal that is
...
resolvent \verb+[~x4,x6]+ are assigned 0. In general, any resolvent
generated by resolving the conflict clause and the antecedent clauses
must evaluate to 0 and we can add it as learned clause.
...
Once 0 is assigned to \p{x021}, the clause \verb+[x021,~x4]+ becomes
a unit clause \verb+[~x4]+ which forces the assignment of 1 to \p{x4}.
But we already know (cf. the implication graph) that those two
assignments are sufficient to cause a conflict. It follows that there is
no point in exploring the other assignments to \p{x031} and \p{x1}: the
decision assignment to \p{x021} and the implied assignment to \p{x4} are
sufficient by themselves to cause a conflict.
...
\chapter{The pigeonhole problem}

The pigeonhole principle states that if we place $n+1$ pigeons in $n$
holes then one hole contains at least two pigeons. Therefore, the
...
The different modes make no difference for the two-hole formula, but for
the three-hole formula the improvements are significant:
...
Six decision assignments are skipped, so there are only three conflict
clauses instead of nine.
...
learn the clause \verb+~p12+.

\begin{figure}
\begin{center}
\includegraphics[keepaspectratio=true,height=.9\textheight]{pigeon2-graph-tb}
\end{center}
...
\begin{verbatim}
[~p12,~p32],    [~p12,p31],    [~p12,~p21],    [~p12,p22],    [~p12]
\end{verbatim}

and the learned clause (with only one literal assigned at this level is
\verb+[~p12]+.

The implication graph for the three-hole formula is shown in
Figure~\ref{pigeon3} (with only numbers for the antecedent clauses).
The node labeled \verb+p22=1@3+ is easily seen to be a dominator and the
clause learned by resolution is \verb+[p43,p33,~p22]+. The bold line
...
such as \cite{mlm}, the literals assigned at lower levels (here,
\verb+p43+ and \verb+p33+ assigned at level~2) are \emph{not} decision
assignments, but assignments implied by unit propagation.

\begin{figure}
\begin{center}
\includegraphics[keepaspectratio=true,height=.9\textheight]{pigeon3-graph-tb-cut}
\end{center}
...
\chapter{The Four-Queens Problem}
...
satisfiable and the satisfying assignments provides the solution:

\begin{verbatim}
[p11=0,p12=0,p13=1,p14=0,p21=1,p22=0,p23=0,p24=0,
 p31=0,p32=0,p33=0,p34=1,p41=0,p42=1,p43=0,p44=0]
\end{verbatim}
where \p{pij} is true if a queen is place in column \p{i} and row \p{j}. 

The statistics for the various algorithms are:
\begin{verbatim}
...
assignments (\p{p11=0}, \p{p12=0}, \p{p13=0}) imply the assignment
\p{p14=1}, which in turn implies more assignments of 0 along its row and
diagonal. The placements of the first queen is shown on the left of
...
The next decision assignment is \p{p21=0} or \p{p21=1}, both of which
lead to conflicts. Let us look at \p{p21=0}. \p{p21=0} implies \p{p22=1}
because there has to be a queen in the second column. The next
implications are \p{p42=0}, \p{p43=1}, \p{p33=0}, \p{p31=1} (see the
right of Figure~\ref{queens}). We leave it to the reader to examine the
sequence of assignments following the decision assignment \p{p21} and to
show that it too leads to a conflict.

Figure~\ref{queens-ig} shows the implication graph with the dominator node
\verb+p22=1@4+ and the cut that defines the learned clause
\verb+[~p22,p32,p34,p41,p44]+. (The image is too small to read, but
you can find the original image in the archive or you can regenerate it
...
it a unit clause \verb+~p22+ and force the assignment of 1 to \p{p22},
regardless of the assignment to \p{p21}. Therefore, non-chronological
backtracking can skip that assignment and continue with the previous
decision assignment. Assigning 1 instead of 0 to \p{p13} leads to a
satisfying assignment by unit propagation alone!
 
\begin{figure}[b]
\begin{center}
\includegraphics[keepaspectratio=true,width=.9\textwidth]{queens-graph-lr-cut}
\end{center}
