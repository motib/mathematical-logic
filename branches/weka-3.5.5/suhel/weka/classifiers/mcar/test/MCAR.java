 ...
 */
public class MCAR implements Cloneable, Serializable, OptionHandler, CapabilitiesHandler {
...
//  protected boolean m_Debug = false;
   ...
   */
  public static MCAR makeCopy(MCAR model) throws Exception {

    return (MCAR)new SerializedObject(model).getObject();
  }
   ...
   * @exception Exception if an error occurs
   */
  public static MCAR [] makeCopies(MCAR model,
					 int num) throws Exception {

    if (model == null) {
      throw new Exception("No model classifier set");
    }
    MCAR [] classifiers = new MCAR [num];
    SerializedObject so = new SerializedObject(model);
    for(int i = 0; i < classifiers.length; i++) {
      classifiers[i] = (MCAR) so.getObject();
    }
    return classifiers;
  }
...
//  
   ...
   */
  protected static void runClassifier(MCAR classifier, String[] options) {
    try {
